...
class GhidraURL(java.lang.Object):
    ...
    * ghidra:////UNCServer/UNCshare/<project-name>[?[/<folder-path>]/[<folderItemName>[#ref]]]
    ...
    NOTE: Avoid using :meth:`URL.getRef() <URL.getRef>` which returns the encoded refereance fragment as it
    appears in the URL.  The proper technique is to use ``URL.toURI().getFragment()``.
    Alternatively, the method :meth:`getDecodedReference(URL) <.getDecodedReference>` may be used.  Encoding of space
    character in URL reference can be inconsistent.  URI encodes space as ``%20`` while 
    ...
    def getDecodedReference(url: java.net.URL) -> str:
        ...
        :param java.net.URL url: URL or null if no ref or URL failure occurs
        """
